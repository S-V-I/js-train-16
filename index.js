...
/**
 * Функція `filterStudentsByGrade` фільтрує студентів за рівнем оцінки та
 * виводить інформацію про них.
 *
 *  grade - Рівень оцінки для фільтрації.
 */
function filterStudentsByGrade(students, grade) {
  // Виведемо в консоль критичну помилку з текстом
  ...
  const gradeAStudents = students.filter((student) => student.grade === grade);
  console.log(gradeAStudents.map((student) => student.name));
}
...
// Завдання 2
/**
 * Функція `logArrayElements` приймає масив та виводить кожен його елемент
 * в консоль.
 *
...
function logArrayElements(arr) {
  ...
  // Якщо arr не є масивом, виведеме в консоль критичну помилоку з текстом
  // "Аргумент має бути масивом!"
  // Повертаємо null, що свідчить про неможливість обробки вхідних даних.
  // Перебираємо кожен елемент масиву.
  // Виводимо в консоль поточний елемент масиву та його індекс в форматі
  // Елемент <індекс>: <значення>
...
// Завдання 3
/**
 * Функція `warnAboutMaxArrayLength` перевіряє, чи перевищує довжина масиву
...
function warnAboutMaxArrayLength(arr, maxLength) {
  ...
  // Якщо довжина масиву перевищує максимальну, виводимо попередження про
  // потенційну проблеми з текстом:
  // "Увага! Довжина масиву перевищує максимально допустиму!".
  // Якщо довжина масиву не перевищує максимальну, виведеме в консоль корисне
  // повідомлення з текстом: "Довжина масиву не перевищує максимально допустиму.".
 ...
 * Вона використовує `console.assert()` для перевірки того, чи обидва масиви
 * є однаковими.
 *
...
function compareArrays(arr1, arr2) {
  ...
  // Оскільки напряму порівняти масиви за допомогою оператора === не можна,
  // перетворимо їх в json і порівняємо
  // Використовуємо `console.assert()`, щоб вивести помилку, якщо масиви не однакові.

  // if (arr1 === arr2) {
  //   console.info("Масиви однакові");
  // } else {
  //   console.error("Assertion failed: Масиви не однакові!");
  // } // так теж працює ;))
...
function buildMatrix(size) {
  ...
  // Якщо size не є числом, виведеме в консоль критичну помилку з текстом
  // "Аргумент має бути числом!".
  // Повертаємо null, що свідчить про неможливість обробки вхідних даних.
  ...
  // Цикл всередені циклу для заповнення колонок матриці, лічильник j має бути
  // від 0 до розміру матриці.
  // Генеруємо випадкове число від 0 до 9 округляємо його до меншого цілого,
  // і додаємо його до рядка.
  // Додаємо рядок до матриці.
  // Виводимо матрицю в консоль як таблицю.
...
// Завдання 7
/**
 * Функція `displayGroupedInfo` виводить інформацію про групу студентів в консоль,
 * організовуючи дані у групи.
 * students - масив об'єктів-студентів.
 */
function displayGroupedInfo(students) {
  // Переберемо об'єкт за допомого for of
...
function validateUserInput(userInput) {
  // Перевіримо умову наявності імені користувача, якщо воно відсутнє виводимо
  // "Помилка: ім'я користувача відсутнє!"
  // Перевіремо умову наявності паролю, якщо він відсутній виводимо
  // "Помилка: пароль відсутній!"
  // Перевірка довжини паролю чи менша вона ніж 8, якщо ні виводимо повідомлення
  // про потенційну проблему з текстом
  // "Попередження: пароль має бути довшим за 8 символів!"

  if (userInput.hasOwnProperty(userInput.username)) {
    console.error("Помилка: ім'я користувача відсутнє!");
  }
  if (userInput.hasOwnProperty(userInput.password)) {
...
// Завдання 9
/**
 * Функція `calculateTotalPrice` обчислює загальну вартість товарів та логує
 * час виконання.
 *
...
function calculateTotalPrice(products) {
  ...
  // Створення змінної total яка буде нашою загальною ціною, початкове значення нуль.
  // Перебираємо кожен об'єкт товару та додаємо ціну товару до загальної вартості.
  // Виведення загальної вартості товарів у форматі
  // "Загальна вартість товарів:", total
...
// Завдання 10
/**
 * Функція `countVowelsAndConsonants` пораховує кількість голосних і
 * приголосних літер у слові.
 *
...
function countVowelsAndConsonants(word) {
  ...
  // якщо так, то виводимио лічильник голосних, інакше приголосних
  // Скидаємо лічильники
  ...
  console.log("Кількість голосних:", vovelsCounter);
  console.log("Кількість приголосних:", consonantsCounter);
}
